...
var ps = (function() {
	...
	return{
		...
		relief:function() {
			...
			for (var i = 0; i < height; i += 1) {
				for (var j = 0; j < width; j += 1) {															
					...
					for (var x = -1; x <= 1; x += 1) {
						for (var y = -1; y <= 1; y += 1) {
							r += curMat[i + x][j + y].r * param[pc];
							g += curMat[i + x][j + y].g * param[pc];
							b += curMat[i + x][j + y].b * param[pc];
							//	console.log(param[pc] + ' * ' + curMat[i + x][j + y].r + ' = '+ (curMat[i + x][j + y].r * param[pc]) + ',r=' +r);
							pc += 1;
						}
					}
					}
		...
		rotate:function(degree) {
			...
			// 两个常数，这样不用以后每次都计算了
			f1 = -0.5 * newW * cosR - 0.5 * newH * sinR + 0.5 * oldW;
			f2 = 0.5 * newW * sinR - 0.5 * newH * cosR + 0.5 * oldH;
			...
			for (var i1 = 0; i1 < newH; i1 += 1) {
				for (var j1 = 0; j1 < newW; j1 += 1) {
					...
					// 计算该象素在源图中的坐标
					var i0 = -j1 * sinR + i1 * cosR + f2 ,
						j0 = j1 * cosR + i1 * sinR + f1 ,
						i = Math.floor(i0),
					    u = i0 - i,
					    j = Math.floor(j0),
					    v = j0 - j,
